...
//GoMsgReader read go protobuf message.
func GoMsgReader(protoPath, module string) (err error) {
	o := "./pbr/read.go"
	...
	fileWriter := code_file_build.NewGoFile("pbr", file,
		"product by tools, should not change this file.", "Author: SureMoon", "")
	fileWriter.Import("github.com/golang/protobuf/proto")
	fileWriter.Import(module + "/pb/smn_dict")
	...
	for _, pm := range list {
		...
		f.Imports(module + "/pb/" + strings.Split(clzName, ".")[0])
